...
class FBref:
    ...
    def get_fbref_league_team_data(self, season_name, league_id, league_name):
        ...
        league_data = pd.read_html(
            f"https://fbref.com/en/comps/{league_id}/{year1}-{year2}/{year1}-{year2}-{league_name}-Stats"
        )
    ...
    def get_fbref_fixtures_and_results(self, season_name, league_id, league_name):
        """Function used to grab fixtures and results table for a specific
        league e.g Premier league"""
        year1, year2 = season_name.split("_")

        fixtures_data = pd.read_html(
            f"https://fbref.com/en/comps/{league_id}/{year1}-{year2}/schedule/{year1}-{year2}-{league_name}-Scores-and-Fixtures"
        )
        fixtures_df = fixtures_data[0]

        fixtures_df[["home_score", "away_score"]] = fixtures_df["Score"].str.split(
            "–", 1, expand=True
        )
    ...
    def get_passing_table(
        self, season_name, league_id, league_name, league_team_dict=None, opponent=False
    ):
        ...
        passing_table_df = passing.merge(pass_types, on=["Squad", "# Pl", "90s"]).merge(
            possession, on=["Squad", "# Pl", "90s"]
        )
    ...
    def get_attacking_table(
        ...
        goal_shot_creation = league_team_dict[f"goal_shot_creation{opponent_data}"]

        attacking_table = standard_stats.merge(shooting, on=["Squad", "# Pl"]).merge(
            goal_shot_creation, on=["Squad", "# Pl", "90s"]
        )
    ...
    def get_team_data_dict(
        ...
        league_table_df = self.get_league_table(
            season_name, league_id, league_name, league_team_dict=league_team_dict
        )
    ...
    def get_seasons_data_per_data_type(
        self,
        stats_data_type,
        season_name_list,
        league_id,
        ...
        """Function used to get data for given stats data type for seasons of interest.
        Example stats_data_type: ['attacking', 'defense', 'passing', 'goalkeeping', 'playing_time']
        """
        if seasons_dict is None:
            seasons_dict = self.get_seasons_dict(
                season_name_list, league_id, league_name
            )

        # grab data per season
        stats_data_type_df_list = []

        for season_name, season_dict in seasons_dict.items():
            for season_data_type, season_data in season_dict["data"].items():
                if season_data_type == "team_data":
                    season_df = season_data[stats_data_type]

                    # set season_name column
                    season_df = season_df.assign(season_name=season_name)
                    stats_data_type_df_list.append(season_df)

        # concat data
        seasons_df = pd.concat(stats_data_type_df_list)
        # transpose data
        seasons_df = seasons_df.set_index(["Squad", "season_name"]).T
        # sort columns based by team
        seasons_df = seasons_df.sort_index(axis=1)
